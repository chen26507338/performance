...
import com.stylefeng.guns.modular.act.service.cmd.JumpTaskCmd;
import com.stylefeng.guns.modular.act.service.creator.ChainedActivitiesCreator;
import com.stylefeng.guns.modular.act.service.creator.MultiInstanceActivityCreator;
import com.stylefeng.guns.modular.act.service.creator.RuntimeActivityDefinitionEntityIntepreter;
import com.stylefeng.guns.modular.act.service.creator.SimpleRuntimeActivityDefinitionEntity;
import com.stylefeng.guns.modular.act.utils.ActUtils;
import com.stylefeng.guns.modular.act.utils.ProcessDefCache;
...
public class ActTaskService  {
    ...
    private Logger logger = LoggerFactory.getLogger(ActTaskService.class);
	/**
	 * 获取待办列表
	 * @return
	 */
	public List<Act> todoList(Act act){
		...
		String userId = String.valueOf(String.valueOf(ShiroKit.getUser().getId()));//ObjectUtils.toString(UserUtils.getUser().getId());
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	...
	public Page<Act> historicList(Page<Act> page, Act act){
		String userId = String.valueOf(ShiroKit.getUser().getId());//ObjectUtils.toString(UserUtils.getUser().getId());
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
					if (il.size() > 0){
						if (StrUtil.isNotBlank(il.get(0).getStartUserId())){
							User user = userService.selectById(il.get(0).getStartUserId());
							if (user != null){
				...
				if (StrUtil.isNotEmpty(histIns.getAssignee())){
					User user = userService.selectById(histIns.getAssignee());
					if (user != null){
						e.setAssignee(histIns.getAssignee());
						e.setAssigneeName(user.getName());
					}
	...
	public Page<Object[]> processList(Page<Object[]> page, String category) {
	    ...
	    List<ProcessDefinition> processDefinitionList = processDefinitionQuery.listPage((int) page.getCurrent(), (int)page.getSize());
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcessOnly(String procDefKey, String title, Map<String, Object> vars) {
		String userId = String.valueOf(ShiroKit.getUser().getId());
